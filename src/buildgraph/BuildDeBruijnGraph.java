...
import pregraph.Count64;
import pregraph.Partition;
...
//		String infile = "/home/gaga/data-scratch/yaelbenari/datas/beeData.fastq";
		...
		int k = 55, pivot_len = 8, bufferSize = 8192, numThreads = 32, hsmapCapacity = 10000000;
		int readLen = 100;
		int numBlocks = 1000;//(int)Math.pow(4, pivot_len);//256; 1000;//
...
//            }
		...
		IOrdering ordering = new LexicographicOrdering(pivot_len);//new UniversalFrequencySignatureOrdering(pivot_len, infile, readLen, bufferSize, true, true);
		pregraph.Partition partition = new Partition(k, infile, numBlocks, pivot_len, bufferSize, readLen, ordering);
